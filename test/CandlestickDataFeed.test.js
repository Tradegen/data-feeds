...
describe("CandlestickDataFeed", () => {
  ...
  describe("#updateDataFeed", () => {
    it("onlyDataProvider", async () => {
      ...
      let tx = dataFeed.updateData(parseEther("1.1"), parseEther("0.9"), parseEther("1"), parseEther("1.05"), parseEther("10"), Number(currentTime) + 1000);
      await expect(tx).to.be.reverted;
    ...
    it("meets requirements; no existing candlesticks", async () => {
        ...
        let tx = await dataFeed.connect(otherUser).updateData(parseEther("1.1"), parseEther("0.9"), parseEther("1"), parseEther("1.05"), parseEther("10"), Number(currentTime) + 1000);
        await tx.wait();
        ...
        expect(currentCandlestick[6]).to.equal(Number(currentTime) + 1000);
    ...
    it("meets requirements; existing candlesticks", async () => {
        ...
        let lastUpdated = await dataFeed.lastUpdated();
        expect(lastUpdated).to.equal(Number(currentTime) + 2);

        let indexTimestamp1 = await dataFeed.getIndexTimestamp(1);
        expect(indexTimestamp1).to.equal(Number(currentTime) + 1);
        ...
        expect(currentCandlestick[6]).to.equal(Number(currentTime) + 70);
  ...
  describe("#aggregateCandlesticks", () => {
    ...
    it("3 candlesticks, request 5", async () => {
        ...
        let tx3 = await dataFeed.connect(otherUser).updateData(parseEther("1.15"), parseEther("0.7"), parseEther("1.15"), parseEther("0.75"), parseEther("50"), Number(currentTime) + 80);
        await tx3.wait();
    ...
    it("60 candlesticks", async () => {
        ...
        let tx = await dataFeed.connect(otherUser).updateData(parseEther("1.1"), parseEther("0.9"), parseEther("1"), parseEther("1.05"), parseEther("20"), Number(currentTime) + 10);
        await tx.wait();
        ...
        for (var i = 0; i < 58; i++) {
            let tx3 = await dataFeed.connect(otherUser).updateData(parseEther("1.3"), parseEther("0.7"), parseEther("1.05"), parseEther("1.25"), parseEther("20"), Number(currentTime) + 70);
            await tx3.wait();
        }
